...
local master = msg['id'] 
    chat_id =  msg.to.id 
    if matches[1] == 'wr' and matches[2] == "photo" and is_momod(msg) then 
                    local oscar = 'mate:'..msg.to.id 
                    redis:set(oscar, true) 
                    local text = 'تم قفل الصور مع تحذير {✔}😈' 
reply_msg(master, text, ok_cb, true) 
elseif matches[1] == 'wr' and matches[2] == 'photo' and not is_momod(msg) then 
local text = 'للمشرفين فقط عزيزي👿' 
reply_msg(master, text, ok_cb, true) 

    elseif matches[1] == 'uwr' and matches[2] == 'photo' and is_momod(msg) then 
      local oscar = 'mate:'..msg.to.id 
      redis:del(oscar) 
    local text = ' تم الغاء قفل الصور مع تحذير {❎}😈' 
reply_msg(master, text, ok_cb, true) 
